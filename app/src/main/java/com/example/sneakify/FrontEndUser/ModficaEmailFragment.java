...
//Classe che permette di modificare l'email

public class ModficaEmailFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_modfica_email, container, false);
    ...
    public void setupLayout(View view){
        etEmail = view.findViewById(R.id.etModificaEmailProfilo);
        etVecchiaPassword = view.findViewById(R.id.etPassLogin);
        bSave = view.findViewById(R.id.buttonSaveEmail);
        ...
        bSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                }else if(etVecchiaPassword.getText().toString() == null){
                    Toast.makeText(getContext(), "Attenzione devi inserire la password", Toast.LENGTH_SHORT).show();
                }
                if(etEmail.getText().toString() != null || etVecchiaPassword.getText().toString() != null){
                    AuthCredential credential = EmailAuthProvider
                            .getCredential(user.getEmail(), etVecchiaPassword.getText().toString()); // creo un oggetto di tipo EmailAuthProvider in modo tale da poter cambiare l'email utilizzando la password
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        user.updateEmail(etEmail.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    Toast.makeText(getContext(), "Email Aggiornata", Toast.LENGTH_SHORT).show();
                                                    myRef.child("users").child(user.getUid()).child("email").setValue(etEmail.getText().toString());
                                                } else {
                                                    Toast.makeText(getContext(), "Attenzione! Email non aggiornata, controlla i campi.", Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                                    } else {
